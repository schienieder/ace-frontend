...
import { fetchCateringForecast } from '../../../redux/forecast/forecast.slice'
import { useDispatch, useSelector } from 'react-redux'
import BeatLoader from 'react-spinners/BeatLoader'

const CateringForecast = () => {

    const dispatch = useDispatch()
    const { catering_forecast, isLoadingForecast } = useSelector(state => state.forecastState)

    useEffect(() => {
        dispatch(fetchCateringForecast())
    }, [dispatch])

    return (
        <div className="w-full card flex flex-col gap-y-3">
            <h4 className="text-gray-800 dark:text-gray-300 font-bold">Catering Satisfaction</h4>
            {
                isLoadingForecast ?
                ...
                <ResponsiveContainer width="100%" height={400}>
                    ...
                    >
                        ...
                        />
                        <Line 
                            dataKey="forecast_catering_rate" 
                            stroke="#FBBF24"
                            fill="url(#color2)" 
                            activeDot={{ r: 5 }}
                        />
