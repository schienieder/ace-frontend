...
export default function messages({ partnerProfile, partnerRooms, groupRoomsList }) {
                        ...
                        <div className="card w-full grid grid-cols-custom-layout gap-x-5">
                            ...
                            <div className="col-start-1 rounded-md flex flex-col border border-gray-300 p-5 gap-y-3">
                                <div className="flex flex-col gap-y-2">
                                    ...
                                    <button 
                                        className={`${ partnerStyles.addBtn } justify-center color-transition`}
                                        onClick={ openModal }
                                    >
                                        <svg 
                                            xmlns="http://www.w3.org/2000/svg" 
...
export const getServerSideProps = async ({ req }) => {
    ...
    const res2 = await fetch(`${api}all_partner_groups/`, {
        method : 'GET',
        headers : {'Authorization' : 'Bearer'+' '+token}
    })
    const data2 = await res2.json()
    const res3 = await fetch(`${api}all_group_rooms/`, {
        method : 'GET',
        headers : {'Authorization' : 'Bearer'+' '+token}
    })
    const data3 = await res3.json()
    const partnerRooms = data2.results.filter((room) => {
        return room.partner === data1.id
    }) 
    return {
        props : {
            partnerProfile : data1,
            partnerRooms : partnerRooms,
            groupRoomsList : data3
        }
    }
}